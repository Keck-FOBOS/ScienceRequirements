...
%% Additional Pacakages, not yet in sty
\usepackage{paralist}
\usepackage{amsmath}
\usepackage{amssymb}
...
%% GRAPHICS
% Each SDN can carry graphics. Place the pathname of your SDN here. It helps to keep your SDN self-contained, so that this doc is the top-level one. Any sub-dirs (subfolders for Windows users) should be referenced relative to the top level.
% Additionally, for large (>5 MB) graphics, please resize so that they are suitable for printing (120 dpi preferred) without being too large.
% You can have multiple (unused) graphics directories with the Originals, Resized, etc.
%\graphicspath{
%{./Figures/}, % base filepath is Figures
%{./Figures/Logo/}, % Logo filepath
%{./Figures/Resized/} % Figures used in this SDN should live here
%}

%% ALIASES
\input{aliases}

%% ABSTRACT Page values
...
\begin{description}
%
\item[S1.1] Redshift precision of 150 \kms{} or better. Stage IV cosmology experiments need to constrain the mean redshift of shear bins to $\lesssim0.2\%$. Redshift samples used for calibration must achieve at least this level of precision per source, which sets this requirement. 
%
\item[S1.2]  Redshift success rate of 75\% measured in a magnitude bin of width 0.5 mag up to the magnitude limit.  Explanation...
%
\item[S1.3] Magnitude limit of $i_{\rm AB} = 25.3$.  This limit corresponds to the expected depth of LSST lensing samples (REF).
%
\item[S1.4] Minimum sample size of 15,000, required for sufficient coverage of the photometric color space and for training statistics (REF).
%
\item[S1.5] 10 independent fields of minimum 20\arcmin{} diameter in order to average over systematics in the galaxy population owing to sample variance.
%
\item[S1.6] Redshift range of $z = $[0.1, 3.5], spanning the distribution of LSST and WFIRST cosmology samples (REF).
%
\end{description}
...
\item[S4] Radial velocities to $\lesssim$1 km/s.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] $\text{S/N}>XX$
\item[S4] [Fe/H] and [$\alpha$/Fe] measured to $\lesssim$0.1 dex.
\item[S4] Individual elemental abundances measured to $\lesssim$0.2 dex.
\item[R4] Spectral coverage from XXXX-XXXX \AA.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] $\text{S/N}>XX$
\item[S4] Stellar ages from CN absorption features to $\lesssim$ XX Gyr.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] Spectral coverage from XXXX-XXXX \AA.
\item[R4] $\text{S/N}>XX$
...
\item[P3.] Sample size (M31 Disk): 100,000 RGB stars and 500 young star clusters
\item[P3.] Sample size (M33 Disk): 10,000 RGB stars
\item[P3.] Sample size (M31 Satellites): 9,000 RGB stars

\item[P3.] Selection (M31 Disk): 6 disk pointings in the PHAT region between 5-20 kpc, each visited 10 times. 2 disk pointings beyond 20 kpc, each visited once. Every visit will target a unique set of stars with $\sim$95\% completeness.
\item[P3.] Selection (M33 Disk): 3 disk pointings, each visited twice. Each visit will target a unique set of stars with $\sim$95\% completeness.
\item[P3.] Selection (M31 Satellites): 3 pointings in the region of NGC 147, NGC 185, and And II, each visited once. 1 pointing for NGC 205, visited twice. Each visit will target a unique set of stars with $\sim$95\% completeness.

\item[P3.] Instrument configuration (M31 Disk): $>$70\% MOS fibers and $<$30\% IFU fiber bundles ($\sim$5.6" diameter)
\item[P3.] Instrument configuration (M33 Disk): 100\% MOS fibers.
\item[P3.] Instrument configuration (M31 Satellites): 100\% MOS fibers.

\item[P3.] Exposure time: 10 hours of integration per visit.
\item[P3.] Duration: 19 nights per year over 5 years for a total of 95 nights (84 for the M31 disk, 6 for the M33 disk, and 5 for M31 satellites).
