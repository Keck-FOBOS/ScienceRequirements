...
\begin{programrequirement}
\reqitem The total area shall be greater than 6 deg$^2$ to survey sufficient volume for detection of rare, proto-cluster structures which will evolve into $z=0$ halos with 10$^{14}$ \msun{}.  This total area will be divided into 12 sub-fields for ease of scheduling.  Each sub-field shall be at least $\sim$0.5 deg$^2$ in order to span the largest structures \citep[10 Mpc; see][]{lee16}.
...
\reqitem To detect a minimum \ion{H}{1} column density of $10^{13}$ \edit{JB: I don't think this could be realistic esp. with S/N spec. Need to track down where this came from. Wait okay, this checks out.} with a statistical error of no more than 0.1 dex (25\%)\edit{S/N=3.5 delivers $10^{13}$ at 3-$\sigma$}, 3-hour integrations must reach a ${\rm S/N} \sim 3.5$ at $r_{\rm AB} \approx 24.6$ \edit{still need to check this against ETC}. 
