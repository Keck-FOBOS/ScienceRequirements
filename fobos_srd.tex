...
\subsubsection{Science Requirements: Assembly and Evolution of Andromeda's Disk and Satellite Galaxies}

\begin{description}

\item[S4] Radial velocities to $\lessim$1 km/s.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] $\text{S/N}>XX$
\item[S4] [Fe/H] and [$\alpha$/Fe] measured to $\lessim$0.1 dex.
\item[S4] Individual elemental abundances measured to $\lessim$0.2 dex.
\item[R4] Spectral coverage from XXXX-XXXX \AA.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] $\text{S/N}>XX$
\item[S4] Stellar ages from CN absorption features to $\lessim$ XX Gyr.
\item[R4] Spectral resolution of $R>XXXX$
\item[R4] Spectral coverage from XXXX-XXXX \AA.
\item[R4] $\text{S/N}>XX$
